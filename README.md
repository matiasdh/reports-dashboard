...
- **Job retry and idempotency:** `ReportGeneratorJob` uses `retry_on StandardError, wait: :polynomially_longer, attempts: 3` for transient failures, and `discard_on ActiveRecord::RecordNotFound` plus `discard_on ActiveJob::DeserializationError` for edge cases (e.g. report deleted before the job runs). `GenerateReportService` guards against duplicate work with `return success unless report.pending?`, so re-running the job (e.g. on retry) does not re-execute work for reports already completed, failed, or processing.
