...
- **`result_data` as `text` instead of `jsonb`:** Report data is stored as a plain `text` column with JSON content, rather than PostgreSQL's `jsonb` type. For this MVP the column is only ever written once and read back in full (parsed by `ReportPdfRenderer`), so there is no need for JSON indexing, partial queries, or DB-level validation. In a production evolution we would migrate to `jsonb` to enable `GIN` indexing, JSON path queries, and stronger data integrity at the database layer.
...
- **`result_data` parsing responsibility:** `ReportPdfRenderer` directly calls `JSON.parse(report.result_data)`. In a larger codebase, it would be better to encapsulate this in a model method like `Report#parsed_data` or use `store_accessor` for typed attribute access, centralizing the parsing logic. For this project, however, the single call in the renderer remains simple and clear.
